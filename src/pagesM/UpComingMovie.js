...
export default function UpComingMovie() {
    ...
    const upComingM = async (pageNumber)=>{
        const get = await axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=1b6ccfb407b0626e097c87368fba764e&language=en-US&with_genres=${getId}&certification=${addCerReduce}&primary_release_date.gte=${formattedStringDate}&primary_release_date.lte=${formattedStringDateEnd}&certification_country=US&page=${pageNumber}`);
    ...
    const handleScroll = debounce(() => {
      ...
      const handleGenreChange = async (selectedGenre) => {
        const initialMovies = await upComingM(1);
        setMov(initialMovies);
    ...
    useEffect(() => {
        if (page > 1) {
          const fetchMovies = async () => {
            const newMovies = await upComingM(page);
